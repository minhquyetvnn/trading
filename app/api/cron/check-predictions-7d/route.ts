...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('\n🕐 [CRON] Starting 7d predictions check...');
    ...
    // -------------------------
    // Thiết lập khoảng thời gian 7 ngày
    // -------------------------
    const checkTime = new Date();
    checkTime.setDate(checkTime.getDate() - 7); // bắt đầu 7 ngày trước
    checkTime.setMinutes(checkTime.getMinutes() - 5); // buffer 5 phút

    const upperBound = new Date();
    upperBound.setMinutes(upperBound.getMinutes() + 30); // buffer 30 phút

    // Lấy các prediction 7d chưa được check
    ...
    console.log(`📊 Found ${predictions?.length || 0} predictions to check (7d)`);
    ...
    for (const pred of predictions) {
      try {
        ...
        await updatePredictionResult(pred.id, '7d', currentPrice);
...
📅 <b>7D Predictions Check Complete</b>
